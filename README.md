...
- 秒（s）単位の `経過時間` 列を合成し、`metadata.xAxisId` に設定する。記録のクロックには誤差があり `msec` も1サンプルごとの経過ミリ秒ではないため、`時間`（`HH:MM:SS`）の秒差を全体の経過時間として、その間に全行を等間隔で並べる（3,000行の記録なら30秒÷2,999で約10ms間隔）。求めた間隔は `details.sampleIntervalMs` で確認できる
