...
- `時間`（`HH:MM:SS`）と `msec` から経過ミリ秒の `経過時間` 列を合成し、`metadata.xAxisId` に設定する。グラフX軸では秒（s）表示、日をまたぐ記録は24時間を加算して単調増加させる
...
- Trace形式は`時間`＋`msec`から`経過時間`（ms）を合成し、記録全体の秒数をOverviewに表示
- Trace形式の3行目の分類は省略（空欄）を直前の値で補完し、列グループとして表示
