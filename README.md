...
- 秒（s）単位の `経過時間` 列を合成し、`metadata.xAxisId` に設定する。記録のクロックには誤差があり `msec` も1サンプルごとの経過ミリ秒ではないため、`時間`（`HH:MM:SS`）の秒差を全体の経過時間として、その間に全行を等間隔で並べる。求めた間隔は記録側の刻み（3,000行=30秒なら10ms）へ丸めるので、値は `0.01, 0.02, 0.03 …` と桁数が揃う。`時間` がミリ秒まで持つ形式（Trace CSV 2）はその値をそのまま使う。採用した基準と間隔は `details.timeBase` と `details.sampleIntervalMs` で確認できる
